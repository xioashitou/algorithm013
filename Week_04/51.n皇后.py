...
class Solution:
    def solveNQueens(self, n: int) -> List[List[str]]:
        res = []
        def helper(row=0, queens=[], xy_subs=[], xy_sums=[]):
            if row == n:
                res.append(queens)
                return

            for col in range(n):
                if  col not in queens and row - col not in xy_subs and row + col not in xy_sums:
                    helper(row + 1, queens + [col], xy_subs + [row - col], xy_sums + [row + col])
